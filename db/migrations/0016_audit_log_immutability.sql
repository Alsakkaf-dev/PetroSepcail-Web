...
-- detected." 0006_rls_policies.sql granted service_role blanket
...
-- RLS's own `force row level security` on audit.audit_log gives service_role
...
-- grant at all regardless), but never re-revoked them from service_role
-- after 0006's blanket grant. Narrow it here: service_role keeps SELECT/
...
grant update, delete on audit.audit_log to service_role;
