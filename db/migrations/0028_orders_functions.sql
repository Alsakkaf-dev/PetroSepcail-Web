...
-- Additive params beyond the doc's base signature: p_fulfillment_type/
-- p_pickup_location_id (D-14b), p_delivery_fee/p_discount_amount (computed
-- by the caller from the EP-X-005/EP-X-002 seams before this call — this
-- function's own job is price/stock authority + atomicity, not quoting).
-- Idempotent on (user_id, idempotency_key): a retried key returns the
-- original order (is_replay=true), never a second row.
