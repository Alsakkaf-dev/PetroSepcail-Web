...
-- 04-database-design.md were written against `auth.jwt()`; this codebase
-- calls the renamed `app_auth.jwt()` directly, so this must exist before
-- any policy is created.
-- SPEC-GAP (managed-Postgres migration): named `app_auth`, not `auth` — on
-- Supabase, `auth` is a reserved vendor schema (Supabase Auth/GoTrue) and
-- cannot be created or repurposed. Every call site uses `app_auth.jwt()`.
create schema app_auth;
comment on schema app_auth is 'PC-03/PC-04 — self-hosted RLS claim-reader helpers; not a vendor schema.';

create function app_auth.jwt() returns jsonb
language sql stable
as $$
  select nullif(current_setting('request.jwt.claims', true), '')::jsonb
$$;
comment on function app_auth.jwt() is
...
drop function if exists app_auth.jwt();
