...
create policy order_insert_own on orders.orders
  for insert with check (user_id = (auth.jwt()->>'sub')::uuid and kind = 'retail');
...
grant all privileges on all tables in schema orders to service_role;

-- Down Migration

revoke all privileges on all tables in schema orders from service_role;
