...
-- service_role bypasses RLS (BYPASSRLS) for workers (0003_app_roles.sql).
grant usage on schema core to app_user, service_role;
grant usage on schema audit to service_role;
grant usage on schema auth to app_user, service_role;
...
create policy role_grants_self_read on core.role_grants
  for select using (identity_id = (auth.jwt()->>'sub')::uuid);
...
-- policies; only service_role"); mfa_secrets/verification_tokens hold raw
...
create policy addr_own on core.addresses           -- "Customer address book: CRUD own"
  for all using (identity_id = (auth.jwt()->>'sub')::uuid)
...
create policy consents_self_insert on core.consents
  for insert with check (identity_id = (auth.jwt()->>'sub')::uuid);

grant select, insert on core.consents to app_user;

-- ---------------------------------------------------------------------------
-- core.outbox — no end-user policies (spec explicit: service_role only).
-- ---------------------------------------------------------------------------
...
-- No end-user policies: internal delivery-attempt log, service_role only.
...
-- service_role (e.g. checkout VAT calc), not fetched raw by the browser.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- service_role: full bypass-backed access to every table created so far.
-- ---------------------------------------------------------------------------
...
revoke all privileges on all tables in schema audit from service_role;
revoke all privileges on all tables in schema core from service_role;
...
revoke usage on schema auth from app_user, service_role;
revoke usage on schema audit from service_role;
revoke usage on schema core from app_user, service_role;
